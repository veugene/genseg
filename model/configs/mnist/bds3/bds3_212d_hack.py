...
class munit_msg_discriminator(nn.Module):
    ...
    def _make_net(self):
        ...
        #layer = norm_nlin_conv(in_channels=ch1,
                               ...
                               #norm_kwargs=self.norm_kwargs)
        #cnn.append(layer)
        layer = norm_nlin_conv(in_channels=ch1//2,
                               out_channels=1,
                               kernel_size=1,
                               nonlinearity=self.nonlinearity,
                               normalization=self.normalization,
    ...
    def forward(self, x_list):
        ...
        for cnn, converter in zip(self.cnns, self.converters):
            f = None
            for i, (layer, convert) in enumerate(zip(cnn[:-1], converter)):
                x = x_list[i]
                y = convert(x)
                if f is None:
                    f = y
                    #print("DEBUG @ {}: {}".format(i, y.size()))
                else:
                    #print("DEBUG @ {}: {}, {}".format(i, y.size(), f.size()))
                    f = torch.cat([y, f], dim=1)
                f = layer(f)
            #y = converter[-1](x_list[i+1])
            #print("DEBUG @ -1:", y.size(), f.size())
            #f = torch.cat([y, f], dim=1)
            #f = self.batch_discriminator(f)
            #f = cnn[-2](f)
            out = cnn[-1](f)
