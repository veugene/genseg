...
from model.common.network.basic import (adjust_to_size,
                                        ...
                                        tiny_block,
                                        munit_discriminator)
from model.bmsg.gan import Discriminator
...
def build_model():
    ...
    submodel = {
        ...
        'mutual_information': mi_estimation_network(
                                            ...
                                            n_hidden=1000),
        #'disc_A'            : msg_discriminator(**discriminator_kwargs),
        #'disc_B'            : msg_discriminator(**discriminator_kwargs)}
        #'disc_A'            : munit_discriminator(**discriminator_kwargs),
        #'disc_B'            : munit_discriminator(**discriminator_kwargs)}
        'disc_A'            : Discriminator(depth=4),
        'disc_B'            : Discriminator(depth=4)}
...
class msg_discriminator(nn.Module):
    def __init__(self, input_dim, num_channels_list,
                 normalization=None, norm_kwargs=None, kernel_size=4,
                 nonlinearity=lambda:nn.LeakyReLU(0.2, inplace=True),
                 padding_mode='reflect', init=None, skip=True):
        super(msg_discriminator, self).__init__()
        ...
        block_kwargs = {'skip': True,
                        ...
                        'init': init,
                        'nonlinearity': nonlinearity}
        for i, (ch0, ch1) in enumerate(zip(self.num_channels_list[:-2],
                                           self.num_channels_list[1:-1])):
            self.converters.append(
                #convolution(in_channels=input_dim,
                            #out_channels=ch0//2,
                            #kernel_size=1,
                            #init=init)
                convolution(in_channels=input_dim,
            ...
            self.layers.append(
                #basic_block(in_channels=ch0 if i else ch0//2,
                            #num_filters=ch1-ch1//2,
                            #kernel_size=self.kernel_size,
                            #subsample=True,
                            #**block_kwargs)
                basic_block(in_channels=ch0,
                            num_filters=ch1,
                            kernel_size=self.kernel_size,
                            subsample=True,
                            **block_kwargs)
                #norm_nlin_conv(in_channels=ch0,
                               #out_channels=ch1,
                               #kernel_size=self.kernel_size,
                               #subsample=True,
                               #conv_padding=True,
        ...
        self.final_downscale_layer = basic_block(
            in_channels=self.num_channels_list[-2],
            num_filters=self.num_channels_list[-1],
            kernel_size=self.kernel_size,
            subsample=False,
            **block_kwargs)
        #self.final_downscale_layer = norm_nlin_conv(
            #in_channels=self.num_channels_list[-2],
            ...
            #conv_padding=True,
            #**block_kwargs)
        self.out_layer = norm_nlin_conv(in_channels=self.num_channels_list[-1],
                                        out_channels=1,
                                        kernel_size=1,
                                        nonlinearity=self.nonlinearity,
                                        normalization=self.normalization,
    ...
    #def forward(self, x_list):
            ...
            #x_list = [x_list] + [F.avg_pool2d(x_list, int(np.power(2, i)))
                                 #for i in range(1, len(self.converters))]
        #else:
            #x_list = x_list[::-1]
        #f = None
        #for x, convert, layer in zip(x_list, self.converters, self.layers):
            #y = convert(x)
            #if f is None:
                #f = y
            #else:
            ...
            #f = layer(f)
        #f = self.final_downscale_layer(f)
