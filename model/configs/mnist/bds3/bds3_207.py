...
class guide_block(nn.Module):
    def __init__(self, channels, width, height):
        ...
        self.height = height
        m = 0.01
        self.s_c = nn.Parameter(torch.empty(1,channels,1,1).uniform_(-m, m))
        self.s_w = nn.Parameter(torch.empty(1,1,width,1).uniform_(-m, m))
        self.s_h = nn.Parameter(torch.empty(1,1,1,height).uniform_(-m, m))
        self.b   = nn.Parameter(torch.empty(1,channels,1,1).uniform_(-m, m))
