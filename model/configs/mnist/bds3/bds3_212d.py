...
def build_model():
    ...
    submodel = {
        ...
        'mutual_information': mi_estimation_network(
                                            ...
                                            n_hidden=1000),
        'disc_A'            : munit_msg_discriminator(**discriminator_kwargs),
        'disc_B'            : munit_msg_discriminator(**discriminator_kwargs)}
    ...
    def __init__(self, input_shape, output_shape, num_conv_blocks, block_type,
        ...
        for n in range(self.num_conv_blocks):
            ...
            if   self.long_skip_merge_mode=='skinny_cat':
                ...
                pass
        '''
        Set up conversion of features to images.
        '''
        self.converters = nn.ModuleList()
        for ch in self.num_channels_list[:-1]:
            if   self.long_skip_merge_mode=='skinny_cat':
                in_ch = ch+1
            elif self.long_skip_merge_mode=='cat':
                in_ch = ch*2
    ...
    def forward(self, z, skip_info=None, mode=0):
        ...
        for n, block in enumerate(self.blocks):
            ...
            if n < self.num_conv_blocks-1:
                out_image = self.converters[n](out)
                out_list.append(out_image)
                #print("DEBUG appending", out_image.size())
        out = self.pre_conv(out)
        out = self.out_conv[mode](out)
        if mode==0:
            out = torch.tanh(out)
            out_list.append(out)
            #print("DEBUG appending", out.size())
            return out_list, skip_info
        elif mode==1:
...
class munit_msg_discriminator(nn.Module):
    def __init__(self, input_dim, num_channels_list, num_scales=3,
        ...
        super(munit_msg_discriminator, self).__init__()
        ...
        self.padding_mode = padding_mode
        self.init = init
        self.downsample = nn.AvgPool2d(kernel_size,
                                       stride=2,
                                       padding=[1, 1],
    ...
    def _make_net(self):
        cnn = nn.ModuleList()
        converter = nn.ModuleList()
        convert = convolution(in_channels=self.input_dim,
                              out_channels=self.num_channels_list[0],
                              kernel_size=1,
                              init=self.init)
        converter.append(convert)
        layer = convolution(in_channels=self.num_channels_list[0],
                            out_channels=self.num_channels_list[0]//2,
                            kernel_size=self.kernel_size,
                            stride=2,
        ...
        for i, (ch0, ch1) in enumerate(zip(self.num_channels_list[:-1],
            ...
            cnn.append(layer)
        convert = convolution(in_channels=self.input_dim,
                                  out_channels=ch1-ch1//2,
                                  kernel_size=1,
                                  init=self.init)
        converter.append(convert)
        layer = norm_nlin_conv(in_channels=ch1+1,
                               out_channels=ch1,
                               kernel_size=self.kernel_size,
                               nonlinearity=self.nonlinearity,
                               normalization=self.normalization,
                               norm_kwargs=self.norm_kwargs)
        cnn.append(layer)
        layer = norm_nlin_conv(in_channels=ch1,
                               out_channels=1,
                               kernel_size=1,
                               nonlinearity=self.nonlinearity,
                               normalization=self.normalization,
        ...
        return cnn, converter

    def forward(self, x_list):
        if isinstance(x_list, torch.Tensor):
            # If x_list is just a tensor, assume it's a single image and 
            # create a list of downscaled images.
            x_list = [x_list]
            num_downscale = self.num_scales+len(self.num_channels_list)-1
            for i in range(num_downscale):
                x_list.append(self.downsample(x_list[-1]))
            #print("DEBUG real")
        else:
            x_list = x_list[::-1]
            #print("DEBUG fake")
        outputs = []
        for cnn, converter in zip(self.cnns, self.converters):
            #print("DEBUG lengths", len(cnn), len(converter))
            #print("DEBUG x_list", [x.size() for x in x_list])
            f = None
            for i, (layer, convert) in enumerate(zip(cnn[:-2], converter)):
                x = x_list[i]
                y = convert(x)
                #print("DEBUG", x.size(), f.size() if f is not None else None)
                if f is None:
                    f = y
                else:
                    f = torch.cat([y, f], dim=1)
                f = layer(f)
            #print("DEBUG", x_list[i+1].size(), f.size())
            y = converter[-1](x_list[i+1])
            f = torch.cat([y, f], dim=1)
            f = self.batch_discriminator(f)
            f = cnn[-2](f)
            out = cnn[-1](f)
            outputs.append(out)
            ...
            new_x_list = []
            for x in x_list:
                new_x_list.append(self.downsample(x))
            x_list = new_x_list
        return outputs
