...
def build_model():
    ...
    class generator(nn.Module):
        ...
        def __init__(self, d=128):
            ...
            self.conv1 = nn.Conv2d(100, d*8, 3, 1, 1)
            ...
            self.conv5 = nn.Conv2d(d+3, d, 3, 1, 1)
            self.conv5_bn = nn.BatchNorm2d(d+4)
            self.dc5_1 = nn.Conv2d(d+3, 1, 3, 1, 2, dilation=2)
            self.dc5_2 = nn.Conv2d(d+3, 1, 3, 1, 4, dilation=4)
            self.dc5_3 = nn.Conv2d(d+3, 1, 3, 1, 8, dilation=8)
            self.dc5_4 = nn.Conv2d(d+3, 1, 3, 1, 8, dilation=16)
            self.conv6 = nn.Conv2d(d+4, 1, kernel_size=3, stride=1, padding=1)
        ...
        def forward(self, common, residual, unique):
            ...
            x = F.upsample(x, scale_factor=2)
            x = F.leaky_relu(self.conv2_bn(torch.cat([
                    adjust_to_size(
            ...
            x = F.upsample(x, scale_factor=2)
            x = F.leaky_relu(self.conv3_bn(torch.cat([
                    adjust_to_size(
            ...
            x = F.upsample(x, scale_factor=2)
            x = F.leaky_relu(self.conv4_bn(torch.cat([
                    adjust_to_size(
            ...
            x = F.relu(self.conv5_bn(torch.cat([
                    adjust_to_size(
                        self.dc5_1(F.avg_pool2d(x, 4, 1, 2,
                                                count_include_pad=False)),
                        x.size()[2:]),
                    adjust_to_size(
                        self.dc5_2(F.avg_pool2d(x, 8, 1, 4,
                                                count_include_pad=False)),
                        x.size()[2:]),
                    adjust_to_size(
                        self.dc5_3(F.avg_pool2d(x, 16, 1, 8,
                                                count_include_pad=False)),
                        x.size()[2:]),
                    adjust_to_size(
                        self.dc5_4(F.avg_pool2d(x, 32, 1, 16,
                                                count_include_pad=False)),
                        x.size()[2:]),
                    self.conv5(x)], dim=1)
                   ))
