...
def build_model():
    ...
    decoder_kwargs = {
        ...
        'output_transform'    : [output_translate(N//16, 1, 'kaiming_normal_'),
                                 output_segment(N//16, 1, 'kaiming_normal_')],
        'skip'                : False,
        'dropout'             : 0.,
...
class decoder(nn.Module):
    def __init__(self, input_shape, output_shape, num_conv_blocks, block_type,
        ...
        self.output_transform = nn.ModuleList(output_transform)
        self.skip = skip
        self.dropout = dropout
        self.normalization = normalization
        self.norm_kwargs = {} if norm_kwargs is None else norm_kwargs
    ...
    def forward(self, z, skip_info=None, out_idx=0):
        ...
        if self.output_transform[out_idx] is not None:
            out = self.output_transform[out_idx](out)
        return out


class output_segment(nn.Module):
    def __init__(self, in_channels, out_channels, init='kaiming_normal_'):
        super(output_segment, self).__init__()
        ...
        self.model = nn.Sequential(
            ...
            convolution(in_channels=in_channels,
                        ...
                        padding_mode='reflect',
                        init=init),
            get_nonlinearity('Sigmoid'))
    
    def forward(self, x):
        return self.model(x)


class output_translate(nn.Module):
    def __init__(self, in_channels, out_channels, init='kaiming_normal_'):
        super(output_translate, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.init = init
        mid_channels = max(in_channels//2, 1)
