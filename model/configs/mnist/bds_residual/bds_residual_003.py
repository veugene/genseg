...
def build_model():
    ...
    submodel = {
        ...
        'decoder'           : decoder_pair(decoder1_kwargs,
                                           decoder2_kwargs),
...
class decoder_pair(nn.Module):
    def __init__(self, kwargs1, kwargs2):
        super(decoder_pair, self).__init__()
        self.decoder = nn.ModuleList([decoder(**kwargs1),
                                      decoder(**kwargs2)])
    
    def forward(self, z, skip_info=None, out_idx=0):
        return self.decoder[out_idx](z, skip_info)