...
def build_model():
    ...
    n = 256 # Number of features to sample at the bottleneck.
    image_size = (1, 48, 48)
    lambdas = {
        'lambda_disc'       : 1,
        'lambda_x_id'       : 1,
    ...
    encoder_kwargs = {
        'input_shape'         : (N//16,)+tuple(image_size[1:]),
        'num_conv_blocks'     : 5,
    ...
    seg_output_transform = nn.Sequential(nn.ReLU(inplace=True),
                                         convolution(in_channels=N//16,
                                                     out_channels=1,
                                                     kernel_size=7,
                                                     padding=3,
                                                     padding_mode='reflect',
    ...
    decoder_kwargs = {
        ...
        'output_shape'        : (N//16,)+tuple(image_size[1:]),
        'num_conv_blocks'     : 5,
        'block_type'          : conv_block,
        'num_channels_list'   : [N, N//2, N//4, N//8, N//16],
        'output_transform'    : [torch.nn.Tanh(), seg_output_transform],
        'skip'                : False,
        'dropout'             : 0.,
    ...
    sample_decoder_kwargs = {
        'input_shape'         : enc_out_shape,
        'output_shape'        : (n,)+tuple(image_size[1:]),
        'num_conv_blocks'     : 5,
        'block_type'          : conv_block,
        'num_channels_list'   : [N, N//2, N//4, N//8, N//16],
    ...
    submodel = {
        ...
        'decoder_rec'       : decoder(**decoder_rec_kwargs),
...
class decoder(nn.Module):
    def __init__(self, input_shape, output_shape, num_conv_blocks, block_type,
        ...
        if not hasattr(output_transform, '__len__'):
            self.output_transform = [output_transform]
        self.output_transform = nn.ModuleList(self.output_transform)
        self.skip = skip
        self.dropout = dropout
        self.normalization = normalization
        self.norm_kwargs = {} if norm_kwargs is None else norm_kwargs
