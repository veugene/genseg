...
def build_model():
    ...
    class LongSkipEncoder(nn.Module):
        def __init__(self, n_downsample, input_dim, dim, norm, activ, pad_type, kernel_size_end=7, kernel_size=4):
    ...
    class LongSkipDecoder(nn.Module):
        def __init__(self, n_upsample, dim, output_dim, res_norm='adain', activ='relu', pad_type='zero',
                    kernel_size_end=7, kernel_size=5, merge_mode='sum', dropout=None):
            ...
            for i in range(n_upsample):
                dim //= 2
                if self.merge_mode=='skinny_cat' and i>0:
                    print("CAT", _dim, dim)
                ...
                self.layers += [(nn.Upsample(scale_factor=2),
                                Conv2dBlock(_dim, dim, kernel_size, 1, (kernel_size-1)//2, norm='none', activation=activ, pad_type=pad_type))]
                _dim = _prev_dim(dim)
    ...
    class decoder_join(nn.Module):
        ...
        def forward(self, common, unique, skips=None):
    ...
    class encoder_split(nn.Module):
        ...
        def forward(self, x):
            out = self.model(x)
            skips = out[:-1]
            a, b = torch.split(out[-1], [N-n, n], dim=1)
    ...
    encoder_kwargs = {
        ...
        'kernel_size'         : 4,
        'kernel_size_end'     : 7}
    encoder = encoder_split(**encoder_kwargs)
    ...
    submodel['decoder'].apply(weights_init('kaiming'))
