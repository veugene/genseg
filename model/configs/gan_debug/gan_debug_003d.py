...
def build_model():
    ...
    class generator(nn.Module):
        ...
        def __init__(self, d=128):
            ...
            self.conv1 = nn.Conv2d(bottleneck_size[0], d*4, 3, 1, 1)
            self.conv1_bn = nn.BatchNorm2d(d*4)
            self.conv2 = nn.Conv2d(d*4, d*2, 3, 1, 1)
            self.conv2_bn = nn.BatchNorm2d(d*2)
            self.conv3 = nn.Conv2d(d*2, d, 3, 1, 1)
            self.conv3_bn = nn.BatchNorm2d(d)
            self.conv4 = nn.Conv2d(d, 1, kernel_size=3, stride=1, padding=1)
        ...
        def forward(self, common, residual, unique):
            x = F.relu(F.upsample(common, scale_factor=2))
