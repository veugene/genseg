...
def build_model():
    ...
    class discriminator(nn.Module):
        def __init__(self, *args, num_scales=3, **kwargs):
            ...
            for _ in range(self.num_scales):
                self.encoder.append(self._make_net(*args, **kwargs))
        
        def _make_net(self, *args, **kwargs):
            class net(nn.Module):
                def __init__(self):
                    super(net, self).__init__()
                    self.enco = encoder(*args, **kwargs)
                    self.nlin = get_nonlinearity(self.enco.nonlinearity)
                    self.conv = convolution(in_channels=self.enco.out_channels,
                                            out_channels=1,
                                            kernel_size=1,
                                            init=self.enco.init)
                def forward(self, x):
