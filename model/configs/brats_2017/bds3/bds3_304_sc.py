...
#
# NOTE: skips are after relu (like nnunet)
...
def build_model(lambda_disc=3,
                ...
                residual=False,
                long_skip_merge_mode='cat', # cat, skinny_cat, sum
                residual_decoder_kernel=3,  # 3, 5
                upsample_mode='conv'):      # conv, repeat
    ...
    encoder_kwargs = {
        ...
        'nonlinearity'        : lambda : nn.LeakyReLU(0.1, inplace=True),
        'ndim'                : 2}
    ...
    remove_spectral_norm(submodel['decoder_residual'].out_conv[1].op)
...
class encoder(nn.Module):
    def __init__(self, input_shape, num_conv_blocks, block_type,
        ...
        self.add_block(num_filters=self.num_channels_list[0], skip=False)
...
class decoder(nn.Module):
    def __init__(self, input_shape, out_channels, num_conv_blocks, block_type,
        ...
        '''
        self.pre_conv = conv_norm_nlin(in_channels=self.out_channels,
        ...
        kwargs_out_conv = {
            ...
            'init': self.init,
            'ndim': self.ndim}
        self.out_conv = [convolution(**kwargs_out_conv),
                         convolution(**kwargs_out_conv)]     # 1 per mode.
...
class conv_norm_nlin(torch.nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size,
                 ...
                 nonlinearity='ReLU', normalization=instance_normalization,
                 norm_kwargs=None, conv_padding=True, padding_mode='constant',
                 init='kaiming_normal_', ndim=2):
        super(conv_norm_nlin, self).__init__()
        if norm_kwargs is None:
            norm_kwargs = {}
