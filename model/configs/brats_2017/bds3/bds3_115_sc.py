...
class decoder(nn.Module):
    def __init__(self, input_shape, output_shape, num_conv_blocks, block_type,
                 ...
                 init='kaiming_normal_', nonlinearity='ReLU',
                 long_skip_merge_mode=None, ndim=2, seed=None):
    ...
    def forward(self, z, skip_info=None, mode=0):
        ...
        if mode==0:
            # Infilling
            # NOTE: translation is handled in `bd_segmentation`
            r_infilling = out[:, :-1]           # All but last channel.
            p = torch.tanh(r_infilling)
            return p, skip_info
        elif mode==1:
            # Segmentation
            r_segmentation = out[:, -1:]        # Last channel.
            eps = 0
            if self.training:
                eps = torch.randn(*r_segmentation.size(), generator=self.rng)
                eps = eps.to(r_segmentation.device)
            l = torch.sigmoid(r_segmentation + eps)
            return l
        else:
            AssertionError()
