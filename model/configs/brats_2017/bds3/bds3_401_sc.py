...
from fcn_maker.blocks import shortcut
...
class Conv(nn.Module):
    def __init__(self, input_channels, output_channels,
                 conv_op=nn.Conv2d, conv_kwargs=None,
                 *args, **kwargs):  # HACK *args and **kwargs are ignored (these exist to match the nnunet block interface)
        super().__init__()
...
def get_dataset_properties(path=None):
    nnunet_architecture_config_files = {
        ...
        'basic_residual_block': ResidualConvDropoutNormNonlin,
...
class encoder(nn.Module):
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
                 ...
                 max_num_features=None, basic_block=ConvDropoutNormNonlin, basic_residual_block=ResidualConvDropoutNormNonlin,
        ...
        self.conv_blocks_context.append(nn.Sequential(
            StackedConvLayers(input_features, output_features, num_conv_per_stage - 1, self.conv_op, self.conv_kwargs,
                              ...
                              self.nonlin_kwargs, first_stride, basic_block=basic_residual_block),
            StackedConvLayers(output_features, self.final_num_features, 1, self.conv_op, self.conv_kwargs,
                              self.norm_op, self.norm_op_kwargs, self.dropout_op, self.dropout_op_kwargs, self.nonlin,
                              self.nonlin_kwargs, basic_block=basic_residual_block)))
