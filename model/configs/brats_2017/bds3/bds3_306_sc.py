# bds3_305_sc with ks4 discriminator and replicate padding
#
...
def build_model(lambda_disc=3,
    ...
    N = 440 # Number of features at the bottleneck.
    n = 120 # Number of features to sample at the bottleneck.
    n_channels = [int(0.05*N), int(0.1*N), int(0.2*N), int(0.4*N), int(0.8*N), N]
    ...
    decoder_common_kwargs = {
        ...
        'num_conv_blocks'     : 6,
        'block_type'          : conv_block,
        'num_channels_list'   : n_channels[::-1],
        'skip'                : True,
        'dropout'             : 0.,
