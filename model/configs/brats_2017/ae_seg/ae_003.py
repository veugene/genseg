...
# All decoders do max unpooling, alone or in combination with other skips.
def build_model():
        ...
        for m in self.blocks:
            ...
            if out_size[-1]!=size[-1] or out_size[-2]!=size[-2]:
                ...
                if self.skip_pool_indices:
                    skip_idx.append(indices)
                skip_ftr.append(out_prev)
        return out, (skip_ftr, skip_idx)
...
class decoder(nn.Module):
    def __init__(self, input_shape, output_shape, num_conv_blocks, block_type,
        ...
        valid_modes = [None, 'skinny_cat', 'cat']
    ...
    def forward(self, z, skip_info=None):
        ...
        if skip_info is not None:
            skip_ftr, skip_idx = skip_info
            skip_ftr = skip_ftr[::-1]
            skip_idx = skip_idx[::-1]
        for n, block in enumerate(self.blocks):
            if (self.long_skip_merge_mode is not None and skip_info is not None
                                                      and n<len(skip_ftr)+1):
                skip = skip_ftr[n]
                out = block(out, unpool_indices=skip_idx[n])
                ...
                out = adjust_to_size(out, skip_idx[n].size()[2:])
                out = block(out, unpool_indices=skip_idx[n])
