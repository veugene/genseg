...
class decoder(nn.Module):
    def __init__(self, input_shape, output_shape, num_conv_blocks, block_type,
        ...
        self.output_0 = norm_nlin_conv(in_channels=last_channels,
                                       ...
                                       init=self.init,
                                       **out_kwargs)
        self.output_1 = assemble_resunet(
                                         #in_channels=4,
                                         ...
                                         main_block_depth=1,
                                         init_num_filters=8,
                                         init_block=conv_block,
                                         main_block=conv_block,
                                         normalization=self.normalization,
                                         norm_kwargs=None,
                                         nonlinearity=lambda :
    ...
    def forward(self, z, skip_info=None, out_idx=0, img=None):
        out = z
        ...
        if out_idx==0:
            ...
            #out = self.output_1(torch.cat([out.detach(),
                                           #self.output_0(out).detach()],
                                #dim=1))
            #out = self.output_1(torch.cat([img,
                                           #out.detach(),
                                           #self.output_0(out).detach()],
                                           #dim=1))
            #out = self.output_1(img)
            #out = self.output_1(torch.cat([img,
                                           #out.detach()],
                                           #dim=1))
            #if self.training:
                #noise = (torch.randn(*out.size())*0.1).cuda()
            #else:
                #noise = torch.zeros_like(out).cuda()
            #out = self.output_1(torch.cat([img,
                                           #out.detach(),
                                           #self.output_0(out).detach()+noise],
                                           #dim=1))
            #out = self.output_1(torch.cat([img,
                                           #self.output_0(out).detach()+noise],
                                           #dim=1))
            #out = self.output_1(self.output_0(out).detach())
            out = self.output_1(torch.cat([img,
                                           self.output_0(out).detach()],
                                dim=1))
        else:
