...
# Added by Christopher Beckham
#
#
#
class ResnetGeneratorWithSkips(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, gpu_ids=[], padding_type='reflect',
                 n_downsampling=2):
        assert(n_blocks >= 0)
        super(ResnetGeneratorWithSkips, self).__init__()
        ...
        self.preproc = nn.Sequential(*preproc)
        ...
        for i in range(n_blocks):
            transforms.append( ResnetBlock(ngf * mult, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias) )
        self.transforms = nn.Sequential(*transforms)

        self.decoders = []
        for i in range(n_downsampling):
            mult = 2**(n_downsampling - i)
            nfi = ngf*mult if i==0 else ngf*mult*2
            decoder = [nn.ConvTranspose2d(nfi, int(ngf * mult / 2),
    ...
    def forward(self, input):
        if self.gpu_ids and isinstance(input.data, torch.cuda.FloatTensor):
            raise NotImplementedException("I have not implemented this yet!!!")
            #return nn.parallel.data_parallel(self.model, input, self.gpu_ids)
        else:
            inp = self.preproc(input)
            encs = []
            for i in range(len(self.encoders)):
                inp = self.encoders[i](inp)
                encs.append(inp)
            inp = self.transforms(inp)
            enc_r = encs[::-1]
            for i in range(len(self.decoders)):
                if i == 0:
                    dec = self.decoders[i](inp)
                else:
                    dec = torch.cat((enc_r[i], dec), dim=1)
                    dec = self.decoders[i](dec)
            out = self.postproc(dec)
            return out
